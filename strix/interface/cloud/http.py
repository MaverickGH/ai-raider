...
TOPUP_COMMAND = "ai-raider cloud billing topup --credits <count>"
BALANCE_COMMAND = "ai-raider cloud billing credits"
...
def api_token(override: str | None = None) -> str:
    ...
    if not token or not token.strip():
        raise CloudError(
            "not signed in. Run `ai-raider cloud login`, or set STRIX_API_TOKEN.",
            exit_code=EXIT_AUTH,
        )
    return token.strip()
...
def _validate_stored_token_origin(record: dict[str, Any]) -> None:
    ...
    if not isinstance(stored_url, str) or not stored_url:
        raise CloudError(
            "the stored sign-in is not bound to a trusted platform. Run `ai-raider cloud login` "
    ...
    try:
        ...
        raise CloudError(
            "the stored sign-in has an invalid platform binding. Run `ai-raider cloud login` again.",
            exit_code=EXIT_AUTH,
        ) from exc
    if stored_origin != active_origin:
        raise CloudError(
            "the stored sign-in belongs to a different platform. Refusing to send its token; "
            "run `ai-raider cloud login` for the configured platform or supply an explicit token.",
            exit_code=EXIT_AUTH,
        )
