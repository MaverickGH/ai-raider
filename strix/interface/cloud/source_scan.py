...
def _source_cleanup_note(upload_id: str, cleanup_error: BaseException) -> str:
    return (
        ...
        f"Retry with `ai-raider cloud uploads delete {upload_id}`."
    )
...
def _interrupted_source_upload_error(
    ...
    message = (
        ...
        f"{upload_id} was retained. Check `ai-raider cloud scans list` before retrying; if no scan "
        f"was created, run `ai-raider cloud uploads delete {upload_id}`.{retry_note}"
    )
    payload: dict[str, Any] = {
        "error": message,
        "interrupted": True,
...
def _retained_source_upload_error(
    ...
    message = (
        ...
        "Check `ai-raider cloud scans list` before retrying; if no scan was created, clean it up "
        f"with `ai-raider cloud uploads delete {upload_id}`. Linked uploads cannot be deleted."
        f"{retry_note}"
    )
    payload: dict[str, Any] = {}
    exit_code = http.EXIT_ERROR
