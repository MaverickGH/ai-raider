"""`ai-raider cloud` — the managed Strix platform (app.strix.ai) from the terminal.
...
_USAGE_HEADER = """[bold]Usage:[/] ai-raider cloud <command> [arguments]
...
_USAGE_FOOTER = """
Run [bold]ai-raider cloud <command> help[/] to list its verbs. Common read-only
...
def _run_cloud(argv: list[str]) -> int:  # noqa: PLR0911, PLR0912
    """Entry point for ``ai-raider cloud …``. Returns a process exit code."""
    console = Console()
...
def _print_verbs(
    ...
    if as_json:
        ...
        payload: dict[str, object] = {
            "command": f"ai-raider cloud {group}",
            "verbs": verbs,
        }
        if error:
            payload["error"] = error
        ...
        return
    console.print(f"[bold]ai-raider cloud {group}[/] verbs:")
