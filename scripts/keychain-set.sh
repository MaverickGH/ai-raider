...
#   ./scripts/keychain-set.sh AIRAIDER_LLM openai/gpt-5.4  # значение строкой (для не-секретов)
set -euo pipefail
[ "$(uname -s)" = "Darwin" ] || { echo "Нужен macOS Keychain."; exit 1; }
NAME="${1:?Имя: OPENAI_API_KEY | ANTHROPIC_API_KEY | OPENROUTER_API_KEY | AIRAIDER_LLM | LLM_API_BASE}"
ACCOUNT="${USER:-$(id -un)}"
