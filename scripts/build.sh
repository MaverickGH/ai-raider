...
echo -e "${BLUE}🦉 AiRaider Build Script${NC}"
echo "================================"
...
TUI_BINARY="build/sidecar/airaider-tui"
...
uv run pyinstaller airaider.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="airaider-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/airaider.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/airaider.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/airaider" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/airaider" "$RELEASE_DIR/$BINARY_NAME"
