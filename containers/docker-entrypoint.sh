...
if [ -n "${AIRAIDER_HOST_UID:-}" ] && [ "${AIRAIDER_HOST_UID}" != "0" ] && [ "${AIRAIDER_HOST_UID}" != "$(id -u)" ]; then
  exec sudo -E -- bash -c '
    set -e
    gid="${AIRAIDER_HOST_GID:-$AIRAIDER_HOST_UID}"
    old_uid="$1"
    old_gid="$2"
    export PATH="$3"
    shift 3
    sed -i "s|^pentester:x:${old_uid}:${old_gid}:|pentester:x:${AIRAIDER_HOST_UID}:${gid}:|" /etc/passwd
    sed -i "s|^pentester:x:${old_gid}:|pentester:x:${gid}:|" /etc/group
    chown -R "${AIRAIDER_HOST_UID}:${gid}" /home/pentester /app/certs
    chown "${AIRAIDER_HOST_UID}:${gid}" /workspace
    exec setpriv --reuid "${AIRAIDER_HOST_UID}" --regid "${gid}" --init-groups "$0" "$@"
  ' "$0" "$(id -u)" "$(id -g)" "$PATH" "$@"
fi
...
# hostname (rather than an IP literal), set AIRAIDER_CAIDO_ALLOWED_DOMAINS to a
...
if [ -n "${AIRAIDER_CAIDO_ALLOWED_DOMAINS:-}" ]; then
  IFS=',' read -ra _caido_domains <<< "${AIRAIDER_CAIDO_ALLOWED_DOMAINS}"
