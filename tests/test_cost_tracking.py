...
from airaider.report.state import (
    ReportState,
...
def test_streaming_logging_stays_enabled_for_cost_callback() -> None:
    with (
        ...
        patch("airaider.config.models._register_litellm_cost_callback") as register,
    ):
        _configure_litellm_compatibility()
...
def test_cost_callback_reads_openrouter_stream_usage_cost() -> None:
    ...
    with patch("airaider.report.state.get_global_report_state", return_value=report_state):
...
def test_openrouter_stream_handler_records_cost() -> None:
    ...
    assert type(config).__name__ == "_AiRaiderOpenrouterConfig"
