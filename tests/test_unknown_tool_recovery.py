...
Models regularly invent tool names that AiRaider does not register (``read_file``
is a common one, borrowed from other agent frameworks). The SDK's default is to
raise ``ModelBehaviorError``, which ends the whole run: nothing in AiRaider retries
...
from airaider.config.models import _NonStreamingModel
...
def _agent() -> Agent[Any]:
    ...
    return Agent(name="AiRaider", instructions="test", tools=[real_tool], model="gw-model")
