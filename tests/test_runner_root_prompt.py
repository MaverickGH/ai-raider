"""Tests for root scan prompt options in run_airaider_scan.

Verify that ``root_instructions_override`` and ``extra_system_prompt_context``
flow through to the root agent's ``build_airaider_agent`` call.
"""
...
from airaider.tools.mcp import BearerAuth, McpConnectionConfig, McpConnectionRequest
...
def _patch_engine_scaffold(
    ...
    """Stub out everything around build_airaider_agent and stop at run_agent_loop.

    Returns a dict that will be populated with the kwargs the runner passed to
    ``build_airaider_agent`` for the root agent.
    """
    ...
    def _build_airaider_agent(**kwargs: Any) -> object:
        if kwargs.get("is_root") and "kwargs" not in captured:
            captured["kwargs"] = kwargs
        return object()

    monkeypatch.setattr(runner, "build_airaider_agent", _build_airaider_agent)
