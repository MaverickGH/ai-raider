...
def tui_executable() -> str:
    return "airaider-tui.exe" if os.name == "nt" else "airaider-tui"
...
async def _launch_posix_tui_process(
    ...
    try:
        env["AIRAIDER_TUI_FD"] = str(child_socket.fileno())
...
async def _launch_windows_tui_process(
    ...
    try:
        ...
        env.update({"AIRAIDER_TUI_ADDR": f"{host}:{port}", "AIRAIDER_TUI_TOKEN": token})
