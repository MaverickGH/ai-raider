...
class _ViewerState:
    def __init__(
        ...
        # standalone ``ai-raider view`` / finished runs, so steering is unavailable.
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``ai-raider view``
    ...
    ``None`` (standalone ``ai-raider view``), steering is reported unavailable.
    """
