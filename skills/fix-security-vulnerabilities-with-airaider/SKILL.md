...
# Fix AiRaider findings and verify

Turn validated AiRaider findings into minimal, correct fixes — and prove they work by re-scanning.
...
Order work by severity: critical → high → medium → low. Every AiRaider finding was validated with a working proof-of-concept, so do not dismiss findings as false positives without re-testing the PoC yourself.
...
## 3. Verify by re-running AiRaider
...
airaider -n -t ./ --scan-mode quick --scope-mode diff --diff-base "$DIFF_BASE" --max-budget 5
