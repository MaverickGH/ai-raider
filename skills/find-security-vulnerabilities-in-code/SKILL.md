...
White-box security review with AiRaider: the agents read the source to build a model of routes, sinks, and authorization checks, then attempt real exploitation. Findings come with a proof-of-concept, so the output is a short list of proven issues rather than the hundreds of "potential" hits a pattern-matching scanner produces.

Install, LLM setup, and all flags are in the **penetration-testing-with-airaider** skill.

## Run it

```bash
# Local working tree
airaider -n -t ./ --scan-mode standard --max-budget 15

# A GitHub repo directly
airaider -n -t https://github.com/org/app --max-budget 15

# Monorepo: point at the service that matters, not the whole tree
airaider -n -t ./services/checkout --max-budget 20

# Only what a branch changed (whole-repo review is wasteful on a large repo)
airaider -n -t ./ --scope-mode diff --diff-base origin/main --max-budget 10
```
...
2. **Scope the review.** Point at the risky subtree and say what matters:
   ```bash
   airaider -n -t ./services/api --max-budget 15 \
...
For diff-scoped review of a branch or PR (and blocking merges on findings), use **ci-security-scanning-with-airaider** — it covers diff scoping, PR comments, and SARIF upload to GitHub code scanning.
...
Hand results to **fix-security-vulnerabilities-with-airaider**: patch the root cause (the shared authorization helper, not the one route), then re-run AiRaider to prove the exploit no longer works.
