...
Install, LLM setup, and all CLI flags live in the **penetration-testing-with-airaider** skill. Read it first if `airaider --version` fails.
...
| Every pull request, continuously | **ci-security-scanning-with-airaider** |
...
Then remediate with **fix-security-vulnerabilities-with-airaider**, which re-runs AiRaider against each fix to prove the exploit no longer works.
