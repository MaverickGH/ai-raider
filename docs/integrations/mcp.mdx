...
AiRaider can connect to [Model Context Protocol (MCP)](https://modelcontextprotocol.io) servers you list and expose their tools to the agent during a run. Use this to let the agent read how your system is actually built instead of inferring it from the outside.
...
Create the file `~/.airaider/mcp-servers.json`. It holds a JSON list of the servers you want the agent to reach. Each entry is either a local `stdio` server that AiRaider launches as a subprocess, or a remote `http` server.
...
Paste the servers you want into `~/.airaider/mcp-servers.json`. The example below shows one of each transport: a local filesystem server over `stdio` and a remote GitHub server over `http` with a bearer token:
...
AiRaider reads this file at the start of each run. There is no default file, so no MCP tools are loaded until you create it. Edit `command`, `args`, `url`, and `token` to match your own servers.
...
<ParamField path="command" type="string">
  For `stdio` servers: the executable AiRaider launches (for example `npx`).
</ParamField>
...
<ParamField path="allowed_tools" type="array">
  ...
  server offers, or set it to a list of tool names to allow only those. AiRaider
...
```bash
airaider --mcp-server github -t ...        # use only the named connection(s)
airaider --mcp-exclude staging-db -t ...   # use everything except the named one(s)
```
...
To read the config from another path instead of `~/.airaider/mcp-servers.json`, either pass `--mcp-config <path>` on the command line:

```bash
airaider --mcp-config ./mcp-servers.json -t ...
```

or set the `AIRAIDER_MCP_CONFIG` environment variable to that path. The flag takes precedence when both are given.

## Startup confirmation

When servers are configured, AiRaider prints a one-line summary at scan startup, for example `MCP: connected 1 server (14 tools): local_fs`, so you can confirm your servers connected.
...
viewer (`airaider view`), so a call that left AiRaider for a server you connected is
