---
title: "Configuration"
description: "Environment variables for AiRaider"
---

Configure AiRaider using environment variables or a config file.

## LLM Configuration

<ParamField path="AIRAIDER_LLM" type="string" required>
...
<ParamField path="AIRAIDER_API_TYPE" type="string">
  ...
  when your gateway requires the other API. Also accepts `AIRAIDER_FORCE_API`.
</ParamField>
...
<ParamField path="AIRAIDER_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="AIRAIDER_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="AIRAIDER_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="AIRAIDER_DEDUPE_MODEL" type="string">
  Model used to judge whether a candidate finding duplicates an existing report.
  Falls back to `AIRAIDER_LLM` when unset.
</ParamField>
...
<ParamField path="AIRAIDER_DEDUPE_REASONING_EFFORT" type="string">
...
<ParamField path="AIRAIDER_WEB_SEARCH_PROVIDER" default="auto" type="string">
  Web search provider: `auto`, `perplexity`, or `exa`. With `auto`, AiRaider uses Exa when `EXA_API_KEY` is set, and Perplexity otherwise. Set an explicit provider to pin one when you configure both keys.
</ParamField>

<ParamField path="AIRAIDER_EXA_SEARCH_TYPE" default="auto" type="string">
...
<ParamField path="AIRAIDER_EXA_NUM_RESULTS" default="5" type="integer">
...
<ParamField path="AIRAIDER_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, AiRaider keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, AiRaider still writes complete run telemetry locally to:

```bash
airaider_runs/<run_name>/events.jsonl
```

When remote vars are set, AiRaider dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="AIRAIDER_IMAGE" default="ghcr.io/usestrix/strix-sandbox:1.3.0" type="string">
...
<ParamField path="AIRAIDER_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="AIRAIDER_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="AIRAIDER_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

AiRaider stores configuration in `~/.airaider/cli-config.json`. You can also specify a custom config file:

```bash
airaider --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "AIRAIDER_LLM": "openrouter/z-ai/glm-5.3",
    "LLM_API_KEY": "sk-...",
    "AIRAIDER_REASONING_EFFORT": "high"
  }
}
```
...
export AIRAIDER_SANDBOX_EXECUTION_TIMEOUT="300"
