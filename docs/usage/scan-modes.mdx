...
AiRaider offers three scan modes to balance speed and thoroughness.

## Quick

```bash
airaider --target ./app --scan-mode quick
```
...
```bash
airaider --target ./app --scan-mode standard
```
...
```bash
airaider --target ./app --scan-mode deep
```
